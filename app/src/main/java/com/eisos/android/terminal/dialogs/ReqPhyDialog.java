...
/**
 * Dialog to request new PhyOptions
 */
public class ReqPhyDialog extends DialogFragment implements View.OnClickListener {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        int preferredPhy = ((DeviceInstanceFragment) getTargetFragment()).getPreferredPhy();
        preferredOption = ((DeviceInstanceFragment) getTargetFragment()).getPreferredOption();
        ...
        btnOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(cbLe1M.isChecked()) {
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY, PhyRequest.PHY_LE_1M_MASK);
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY_OPTIONS, 0);
                } else if(cbLe2M.isChecked()) {
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY, PhyRequest.PHY_LE_2M_MASK);
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY_OPTIONS, 0);
                } else if (cbCoded.isChecked()) {
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY, PhyRequest.PHY_LE_CODED_MASK);
                    data.putExtra(DeviceInstanceFragment.EXTRA_PHY_OPTIONS, preferredOption);
                }

                getTargetFragment().onActivityResult(DeviceInstanceFragment.REQ_PHY, Activity.RESULT_OK, data);
                dismiss();
            }
        });
